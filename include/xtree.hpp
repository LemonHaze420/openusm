...
protected:
    ...
    struct _Node {  // tree node
        _Node(_Genptr _Larg, _Genptr _Parg, _Genptr _Rarg, const value_type &_Val, char _Carg)
            : _Left(_Larg), _Parent(_Parg), _Right(_Rarg), _Myval(_Val), _Color(_Carg), _Isnil(false)
...
#define _TREE_ITERATOR(ppnode) iterator(ppnode)
            ...
            if (_Where == begin()) {                // insert at beginning if before first element
                ...
                if (_Isnil(_Right(_Next._Mynode())))
                    ...
                    return (_Insert(true, _Where._Mynode(), _Val));
                       ...
                       (++(_Next = _Where) == end() || !this->comp(_Key(_Next._Mynode()),
            ...
            if (_Where == begin()) {  // insert at beginning if before first element
                ...
                if (_Isnil(_Right(_Next._Mynode())))
                    ...
                    return (_Insert(true, _Where._Mynode(), _Val));
                       ...
                       (++(_Next = _Where) == end() || this->comp(this->_Kfn(_Val),
    ...
    {  // erase element at _Where
        ...
        if (_Color(_Erasednode) == _Black) {  // erasing black link, must recolor/rebalance tree
            for (; _Fixnode != _Root() && _Color(_Fixnode) == _Black; _Fixnodeparent = _Parent(_Fixnode))
                if (_Fixnode == _Left(_Fixnodeparent)) {  // fixup left subtree
                    ...
                    if (_Isnil(_Pnode))
                        _Fixnode = _Fixnodeparent;  // shouldn't happen
                             ...
                             _Color(_Right(_Pnode)) == _Black) {  // redden right subtree with black children
                        _Color(_Pnode) = _Red;
                        _Fixnode = _Fixnodeparent;
                    } else {                                     // must rearrange right subtree
                        if (_Color(_Right(_Pnode)) == _Black) {  // rotate red up from left sub-subtree
                    ...
                    if (_Isnil(_Pnode))
                        _Fixnode = _Fixnodeparent;  // shouldn't happen
                             ...
                             _Color(_Left(_Pnode)) == _Black) {  // redden left subtree with black children
                        _Color(_Pnode) = _Red;
                        _Fixnode = _Fixnodeparent;
                    } else {                                    // must rearrange left subtree
                        if (_Color(_Left(_Pnode)) == _Black) {  // rotate red up from right sub-subtree
                            _Color(_Right(_Pnode)) = _Black;
                            _Color(_Pnode) = _Red;
                            _Lrotate(_Pnode);
                            _Pnode = _Left(_Fixnodeparent);
    ...
    {                                                                             // free entire subtree, recursively
        for (_Nodeptr _Pnode = _Rootnode; !_Isnil(_Pnode); _Rootnode = _Pnode) {  // free subtrees, then node
    ...
    {  // add node with value next to _Wherenode, to left if _Addnode
        ...
        for (_Nodeptr _Pnode = _Newnode; _Color(_Parent(_Pnode)) == _Red;)
            if (_Parent(_Pnode) == _Left(_Parent(_Parent(_Pnode)))) {  // fixup red-red in left subtree
