...
inline size_t hash_value(const std::basic_string<_Elem, _Traits, _Alloc> &_Str)
...
public:
    ...
    // CLASS iterator
    class iterator : public _Myiterator {  // possibly non-mutable iterator
    public:
        ...
        iterator(const _Myiterator &_Iter) : _Myiterator(_Iter)
    ...
    public:
        _List_position() : _Mypos() {}

        _List_position(iterator _Iter) : _Mypos(_Iter._Mynode(), (_Mylist *)_Iter._Mycont) {}

        _List_position(const_iterator _Iter) : _Mypos(_Iter._Mynode(), (_Mylist *)_Iter._Mycont) {}
    ...
    typedef typename _Mylist::reverse_iterator reverse_iterator;
    typedef typename _Mylist::const_reverse_iterator const_reverse_iterator;
    typedef typename _Mylist::value_type value_type;

    typedef _std::vector<_Myvec_value_type,
                         typename std::allocator_traits<allocator_type>::template rebind_alloc<_Myvec_value_type>>
        _Myvec;
    ...
    explicit _Hash(const key_compare &_Parg, const allocator_type &_Al)
        : _Traits(_Parg), _List(_Al), _Vec(min_buckets + 1, end(), _Al), _Mask(1), _Maxidx(1)
    ...
    _Hash(const _Myt &_Right) : _Traits(_Right.comp), _List(_Right.get_allocator()), _Vec(_Right.get_allocator())
    ...
    {  // try to insert node with value _Val
        ...
        if (_Maxidx <= size() / bucket_size) {  // too dense, need to grow hash table
            ...
            for (_Plist = _Get_iter_from_vec(_Vec[_Bucket]); _Vec[_Bucket + 1] != _Plist;) {  // split old bucket
                ...
                if (_Newbucket == _Bucket) {
                    ...
                    if (++_Pnext != end()) {                                  // not at end, move it
                        for (_Idx = _Bucket; _Vec[_Idx] == _Plist; --_Idx) {  // update end iterators if moving first
                            _Vec[_Idx] = _Pnext;
                            if (_Idx == 0) {
                                break;
                            }
                    ...
                    for (_Idx = _Maxidx; _Bucket < _Idx; --_Idx) {  // update end iterators if new bucket filled
                        if (_Vec[_Idx] != end()) {
                            break;
                        }
                        _Vec[_Idx] = _Plist;
        ...
        for (; _Vec[_Bucket] == _Plist; --_Bucket) {  // update end iterators if new first bucket element
            _Vec[_Bucket] = _Where;
            if (_Bucket == 0) {
                break;
            }
    ...
    {  // find leftmost not greater than _Keyval in nonmutable hash table
        ...
        for (_Where = _Get_iter_from_vec(_Vec[_Bucket + 1]); _Vec[_Bucket] != _Where;)
            if (!this->comp(_Keyval, this->_Kfn(*--_Where)))
                return (this->comp(this->_Kfn(*_Where), _Keyval) ? end() : (const_iterator)++_Where);
        return (end());
    }
...
template <class _Ty>
inline bool operator==(const _Hash<_Ty> &_Left, const _Hash<_Ty> &_Right)
