...
template <class _Ty,
          class _Alloc>
class _Deque_map {  // ultimate base class for deque to hold allocator _Almap
protected:
    _Deque_map(_Alloc _Al) : _Almap(_Al)
    {  // construct allocator from _Al
    }

    typedef typename std::allocator_traits<_Alloc>::template rebind_alloc<_Ty>::value_type *_Tptr;

    typename std::allocator_traits<_Alloc>::template rebind_alloc<_Tptr> _Almap;  // allocator object for maps
};

// TEMPLATE CLASS _Deque_val
template <class _Ty, class _Alloc>
class _Deque_val : public _Deque_map<_Ty, _Alloc> {  // base class for deque to hold allocator _Alval
protected:
    _Deque_val(_Alloc _Al = _Alloc()) : _Deque_map<_Ty, _Alloc>(_Al), _Alval(_Al)
...
public:
    ...
    class const_iterator : public _Ranit<_Ty, _Dift, _Ctptr, const_reference> {  // iterator for nonmutable deque
    public:
    ...
    class iterator : public const_iterator {  // iterator for mutable deque
    public:
        typedef random_access_iterator_tag iterator_category;
        typedef _Ty value_type;
        typedef _Dift difference_type;
        ...
        iterator(size_type _Off, const deque<_Ty, _Alloc> *_Pdeque) : const_iterator(_Off, _Pdeque)
        {  // construct with offset _Off in *_Pdeque
        }
    ...
    deque() : _Mybase(), _Map(nullptr), _Mapsize(0), _Myoff(0), _Mysize(0)
    {  // construct empty deque
    }

    explicit deque(const _Alloc &_Al) : _Mybase(_Al), _Map(nullptr), _Mapsize(0), _Myoff(0), _Mysize(0)
    {  // construct empty deque with allocator
    }

    explicit deque(size_type _Count) : _Mybase(), _Map(nullptr), _Mapsize(0), _Myoff(0), _Mysize(0)
    {  // construct from _Count * _Ty()
        _Construct_n(_Count, _Ty());
    }

    deque(size_type _Count, const _Ty &_Val) : _Mybase(), _Map(nullptr), _Mapsize(0), _Myoff(0), _Mysize(0)
    {  // construct from _Count * _Val
        _Construct_n(_Count, _Val);
    }

    deque(size_type _Count, const _Ty &_Val, const _Alloc &_Al)
        : _Mybase(_Al), _Map(nullptr), _Mapsize(0), _Myoff(0), _Mysize(0)
    ...
    template <class _It>
    deque(_It _First, _It _Last) : _Mybase(), _Map(nullptr), _Mapsize(0), _Myoff(0), _Mysize(0)
    {  // construct from [_First, _Last)
        _Construct(_First, _Last, _Iter_cat(_First));
    }

    template <class _It>
    deque(_It _First, _It _Last, const _Alloc &_Al) : _Mybase(_Al), _Map(nullptr), _Mapsize(0), _Myoff(0), _Mysize(0)
    ...
    {                    // erase element at end
        if (!empty()) {  // something to erase, do it
            ...
            std::allocator_traits<std::decay_t<decltype(this->_Alval)>>::destroy(this->_Alval,
                                                                                 _Map[_Block] + _Newoff % _DEQUESIZ);
            if (--_Mysize == 0) {
                _Myoff = 0;
            }
        }
    ...
    {                             // insert _Val at _Where
        if (_Where == begin()) {  // insert at front
            push_front(_Val);
            return (begin());
        } else if (_Where == end()) {  // insert at back
            push_back(_Val);
            return (end() - 1);
        } else {  // insert inside sequence
            iterator _Mid;
...
protected:
    ...
    {  // free all storage
        ...
        for (size_type _Count = _Mapsize; 0 < _Count;) {  // free storage for a block and destroy pointer
            if (*(_Map + --_Count) != nullptr) {
                std::allocator_traits<std::decay_t<decltype(this->_Alval)>>::deallocate(
                    this->_Alval, *(_Map + _Count), _DEQUESIZ);
            }
        ...
        if (_Map) {
            std::allocator_traits<std::decay_t<decltype(this->_Almap)>>::deallocate(
                this->_Almap, _Map, _Mapsize);  // free storage for map
        }
