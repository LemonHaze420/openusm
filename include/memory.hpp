...
template <class _Ty>
inline std::pair<_Ty _FARQ *, _PDFT> get_temporary_buffer(_PDFT _Count)
{  // get raw temporary buffer of up to _Count elements
    ...
    for (_Pbuf = 0; 0 < _Count; _Count /= 2)
        if ((_Pbuf = (_Ty _FARQ *)operator new((_SIZT)_Count * sizeof(_Ty), std::nothrow_t{})) != 0)
            break;
...
{  // copy [_First, _Last) to raw _Dest, scalar type
    ...
    if (_Count > 0)
        _CRT_SECURE_MEMMOVE(
            &*_Dest, _Count * sizeof(*_First), &*_First, _Count * sizeof(*_First));  // NB: non-overlapping move
    return _Result;
}
...
template <class _FwdIt,
          class _Ty>
class raw_storage_iterator : public _Outit {  // wrap stores to raw buffer as output iterator
public:
...
template <class _Ty>
class _Temp_iterator : public _Outit {  // wrap stores to temporary buffer as output iterator
public:
    ...
    ~_Temp_iterator()
    {                            // destroy the object
        if (_Buf._Begin != 0) {  // destroy any constructed elements in buffer
            for (_Pty _Next = _Buf._Begin; _Next != _Buf._Hiwater; ++_Next)
                _Destroy(&*_Next);
    ...
    {  // assign or construct value into active buffer, and increment
        ...
        else {                          // above high water mark, construct
    ...
    {                                                  // return size of buffer
        if (_Pbuf->_Begin == 0 && 0 < _Pbuf->_Size) {  // allocate buffer on first size query
...
private:
    struct _Bufpar {    // control information for a temporary buffer
...
public:
    ...
    template <class _Other>
    auto_ptr(auto_ptr<_Other> &_Right) _THROW0() : _Myptr(_Right.release())
