...
int nflBufferAlign(nflDriver *driver, nflCommand *ioCommand, uint32_t a3, uint32_t *a4, uint32_t *a5)
{
    assert(ioCommand != nullptr && driver != nullptr && driver->init != nullptr && driver->buffer != nullptr);
        ...
        *a5 = v15;
        ...
        case 4: {
            ...
            if (((buffer->field_4 - 1) & v8) != 0) {
                v18 |= 2u;
                sp_log("nfdBufferAlign: Unaligned transfer of 0x%08X bytes, should be aligned to "
                       "0x%08X\n",
                       v8,
                       buffer[1]);
            }
            ...
            if (v6 != v13) {
                v18 |= 4u;
                sp_log("nfdBufferAlign: Unaligned seek to offset 0x%08X, should be aligned to "
                       "0x%08X\n",
                       v6,
                       buffer[2])
            }
...
static auto &g_WriteFile =
    var<BOOL(__stdcall *)(HANDLE hFile, LPCVOID lpBuffer, DWORD nNumberOfBytesToWrite, LPDWORD lpNumberOfBytesWritten,
                          LPOVERLAPPED lpOverlapped)>(0x009551A4);

static auto &g_WriteFileEx =
    var<BOOL(__stdcall *)(HANDLE, LPCVOID, DWORD, LPOVERLAPPED, LPOVERLAPPED_COMPLETION_ROUTINE)>(0x009551A8);
...
static auto &g_CreateFileA = var<HANDLE(__stdcall *)(
    LPCSTR lpFileName, DWORD dwDesiredAccess, DWORD dwShareMode, LPSECURITY_ATTRIBUTES lpSecurityAttributes,
    DWORD dwCreationDisposition, DWORD dwFlagsAndAttributes, HANDLE hTemplateFile)>(0x00955198);

static auto &g_ReadFile = var<BOOL(__stdcall *)(HANDLE hFile, LPVOID lpBuffer, DWORD nNumberOfBytesToRead,
                                                LPDWORD lpNumberOfBytesRead, LPOVERLAPPED lpOverlapped)>(0x0095519C);

static auto &g_ReadFileEx =
    var<BOOL(__stdcall *)(HANDLE, LPVOID, DWORD, LPOVERLAPPED, LPOVERLAPPED_COMPLETION_ROUTINE)>(0x009551A0);
...
static auto &g_WriteFile = []() -> auto & {
    static BOOL(__stdcall * g_WriteFile1)(HANDLE hFile,
                                          LPCVOID lpBuffer,
...
static auto &g_CreateFileA = []() -> auto & {
    static HANDLE(__stdcall * g_CreateFileA1)(LPCSTR lpFileName,
...
static auto &g_ReadFile = []() -> auto & {
    static BOOL(__stdcall * g_ReadFile1)(HANDLE hFile,
                                         LPVOID lpBuffer,
...
void __stdcall nfd_win32_IoCompletionRoutine(DWORD dwErrorCode, DWORD numberOfBytesTransfered,
...
HANDLE __stdcall sub_81D790(LPCSTR lpFileName, DWORD dwDesiredAccess, DWORD dwShareMode,
                            LPSECURITY_ATTRIBUTES lpSecurityAttributes, DWORD dwCreationDisposition,
                            DWORD dwFlagsAndAttributes, HANDLE a7)
{
    if constexpr (1) {
        ...
        auto result = CreateFileA(
            lpFileName, dwDesiredAccess, dwShareMode, lpSecurityAttributes, dwCreationDisposition, v7, nullptr);
        auto v9 = (int)result;
...
BOOL __stdcall sub_81D820(HANDLE hFile, LPVOID lpBuffer, DWORD nNumberOfBytesToRead, LPDWORD lpNumberOfBytesRead,
                          LPOVERLAPPED a5)
{
    if constexpr (1) {
        WaitForSingleObject(dword_987500, 0xFFFFFFFF);
...
BOOL __stdcall sub_81D950(HANDLE hFile, LPVOID lpBuffer, DWORD nNumberOfBytesToRead, LPOVERLAPPED a4,
...
BOOL __stdcall sub_81DA70(HANDLE hFile, LPCVOID lpBuffer, DWORD nNumberOfBytesToWrite, LPDWORD lpNumberOfBytesWritten,
                          LPOVERLAPPED a5)
{
    if constexpr (1) {
        WaitForSingleObject(dword_987500, 0xFFFFFFFF);
...
BOOL __stdcall sub_81DB90(HANDLE hFile, LPCVOID lpBuffer, DWORD nNumberOfBytesToWrite, LPOVERLAPPED a4,
...
BOOL nfd_win32_IoExecute(nflDriver *a1, HANDLE *a2, nflRequestType requestType, uint32_t a4, LPVOID lpBuffer,
                         uint32_t nNumberOfBytes)
{
    TRACE("nfd_wind32_IoExecute");

    assert(requestType == NFL_REQUEST_TYPE_READ || requestType == NFL_REQUEST_TYPE_WRITE);
    ...
    if constexpr (1) {
        ...
        if (requestType == NFL_REQUEST_TYPE_WRITE) {
            return (g_WriteFileEx(*a2, lpBuffer, nNumberOfBytes, &Overlapped, nfd_win32_IoCompletionRoutine) == 0);
        }

        BOOL result = g_ReadFileEx(*a2, lpBuffer, nNumberOfBytes, &Overlapped, nfd_win32_IoCompletionRoutine) == 0;
