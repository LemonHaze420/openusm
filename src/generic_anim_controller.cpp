    ...
    : nal_anim_controller(a2, a3, a4, a5), field_54(this), field_5C(this)
{
    ...
    if constexpr (1) {
        ...
        Skel->GetComponentHandle<nalPositionOrientation>(this->field_64, v8, v7);

        v8 = {"AE_Base_Bone"};
        v7 = {"shake_root"};
        Skel->GetComponentHandle<nalPositionOrientation>(this->field_74, v7, v8);

        v8 = {"AE_Floor_Offset"};
        v7 = {"fakeroot"};
        Skel->GetComponentHandle<float>(this->field_84, v7, v8);

        v8 = {"NAL_TRAJECTORY"};
        v7 = {"fakeroot"};
        Skel->GetComponentHandle<nalPositionOrientation>(this->field_94, v7, v8);

        v8 = {"USMEvent"};
        v7 = {"fakeroot"};
        Skel->GetComponentHandle<unsigned char>(this->field_A4, v7, v8);

        v8 = {"MaxParamFloat.FOV"};
        v7 = {"camera"};
        Skel->GetComponentHandle<float>(this->field_B4, v7, v8);

        v8 = {"MaxParamFloat.Far Env Range"};
        v7 = {"camera"};
        Skel->GetComponentHandle<float>(this->field_C4, v7, v8);

        v8 = {"MaxParamFloat.Tentacle Base Diameter"};
        v7 = {"fakeroot"};
        Skel->GetComponentHandle<float>(this->field_D4, v7, v8);

        v8 = {"MaxParamFloat.Subtentacle Activity"};
        v7 = {"fakeroot"};
        Skel->GetComponentHandle<float>(this->field_E4, v7, v8);

        v8 = {"MaxParamFloat.Pull Factor"};
        v7 = {"fakeroot"};
        Skel->GetComponentHandle<float>(this->field_F4, v7, v8);
    } else {
...
vector3d generic_anim_controller::get_hint_token_scale(string_hash a2)
{
    ...
    if constexpr (0) {
        ...
        v4->GetComponentHandle(v10, a2.source_hash_code, tlfs_NAL_SCALE);
