...
nglPalette *nglCreatePalette([[maybe_unused]] int a1, uint32_t entries, const void *a3)
{
    ...
    v3->m_palette_entries = static_cast<PALETTEENTRY *>(tlMemAlloc(sizeof(PALETTEENTRY) * entries, 8, 0x1000000));
    ...
    if (!g_valid_texture_format) {
        IDirect3DDevice9_SetPaletteEntries(g_Direct3DDevice, v3->m_palette_idx, v3->m_palette_entries);
    }
