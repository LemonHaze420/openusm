...
void param_block::unmash(mash_info_struct *a1, void *a3)
{
    ...
    if constexpr (1) {
        ...
        if (this->param_array != nullptr) {
            a1->unmash_class(this->param_array,
                             a3
#ifdef TARGET_XBOX
                             ,
                             mash::NORMAL_BUFFER
...
const char *ai::param_block::get_optional_pb_fixedstring(string_hash a2, const char *a3, bool *a4) const
{
    if (a4 != nullptr) {
        *a4 = false;
    }
...
void param_block::param_data_array::initialize(mash::allocation_scope)
{
    this->field_14 = nullptr;
}
    ...
    if constexpr (1) {
        ...
        case PT_FLOAT_VARIANCE:
            this->m_union.float_variance =
                (variance_variable<float> *)a2->read_from_buffer(sizeof(variance_variable<float>), 4);
            break;
        case PT_ENTITY:
        case PT_POINTER:
