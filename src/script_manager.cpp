        ...
        if (script_manager_game_var_container == nullptr) {
            ...
            if (v0 != nullptr) {
                auto allocated_mem = parse_generic_object_mash(
                    script_manager_game_var_container, v0, nullptr, nullptr, nullptr, 0, 0, nullptr);
    ...
    if constexpr (1) {
        for (auto &entry : (*script_manager_execs_pending_link_list)) {
            if ( !entry.exec->is_linked() ) {
                ...
                entry.exec->link();
                script_manager::run_callbacks(
                    static_cast<script_manager_callback_reason>(7), entry.exec, entry.field_8);
            }
    ...
    if constexpr (1) {
        ...
        if (auto it = script_manager_exec_map->find(key); it != script_manager_exec_map->end()) {
            ...
            //if (is_city_arena)
            {
                ...
                auto allocated_mem =
                    parse_generic_object_mash(entry.exec, entry.exec, nullptr, nullptr, nullptr, 0, 0, nullptr);
            ...
            if ((a2 & 1) != 0) {
                if (script_manager_master_script != nullptr) {
                    script_manager::run_callbacks(
                        (script_manager_callback_reason)4, nullptr, "trying to load more than one master script!");
                }
        ...
        script_executable_entry *(*func)(const resource_key *a1, uint32_t a2, void *a3, const resource_key *a4) =
            CAST(func, 0x005B0750);
...
script_object *find_object(const resource_key &a1, const string_hash &a2, const resource_key &a3)
{
    TRACE("script_manager::find_object");
    ...
    if (auto end = script_manager_exec_map->end(); v13 != end) {
        auto &v5 = (*v13);
...
void un_load(const resource_key &a1, bool a2, const resource_key &a3)
{
    ...
    if (auto end = script_manager_exec_map->end(); a1a == end) {
        ;
        auto v9 = mString{"Trying to un load a non-loaded script executable '"} + mString{a1.m_hash.to_string()} +
                  mString{"'"};
...
void *parse_generic_mash_init_hook(generic_mash_header *&header, void *a2, bool *allocated_mem,
                                   generic_mash_data_ptrs *a4, unsigned int struct_size,
                                   unsigned int *virtual_table_lookup, unsigned int *size_table_lookup,
                                   unsigned int num_table_entries, unsigned int base_class_size, void *a10)
{
#if defined(TARGET_XBOX) || defined(OPENUSM_XBPACK_MODE)
    struct_size = 0x60;
#endif

    return parse_generic_mash_init(header,
                                   a2,
                                   allocated_mem,
                                   a4,
                                   struct_size,
...
void script_manager_patch()
{
    ...
    {
        using find_object_t = script_object *(*)(const resource_key &, const string_hash &, const resource_key &);
