...
{
    printf("begin_entry = 0x%08X, arena = 0x%08X, field_8 = 0x%04X, m_size = %d, total_object_count = %d, "
           "alloc_object_count = %d, field_10 = 0x%08X, end_sentry = 0x%08X\n",
           this->begin_sentry,
           int(this->arena),
           this->field_8,
           this->m_size,
...
slab_allocator::slab_t *slab_allocator::find_slab_for_object(void *obj)
{
    ...
    if constexpr (1) {
        ...
        if constexpr (0) {
            sp_log("find_slab_for_object: obj = 0x%08X, static_slab_arena = 0x%08X", obj, static_slab_arena);
        }
        ...
        if ((obj < static_slab_arena) || (obj >= static_slab_arena + 0x400000)) {
            ...
            if (VirtualQuery(slab, &MemInfo, sizeof(MemInfo)) != 0) {
                if (MemInfo.State == MEM_COMMIT && ((MemInfo.Protect & PAGE_READWRITE) != 0)) {
                    if (slab->begin_sentry == SLAB_HEADER_BEGIN_SENTRY && slab->end_sentry == SLAB_HEADER_END_SENTRY &&
                        slab->is_set(SLAB_FROM_AUX | SLAB_FROM_HEAP) && !slab->is_set(SLAB_FROM_STATIC) &&
                        (slab->arena == (char *)uVar3) && slab->contains((uint32_t)obj)) {
                        return slab;
                    } else {
    ...
    for (int i = 0; i < MAX_COUNT; ++i) {
        ...
        if (slab != nullptr) {
            ...
            for (auto *v49 : (*slab_partial_list)) {
                ...
                debug_print_va("    Slab %d: Allocated_Objects = %d, Free_Objects = %d, Utilization = %02.02f",
                               v53,
                               v13,
                               v27,
                               v28);
