...
int Settings::sub_81D010(const char *Source, int default_value)
{
    if constexpr (1) {
        ...
        DWORD cbData = 4;
        if (!this->sub_81CDC0(Source, &Type, (LPBYTE)&Data, &cbData) || (result = Data, Type != REG_DWORD)) {
...
bool Settings::sub_81CDC0(const char *Source, OUT OPTIONAL LPDWORD lpType, OUT OPTIONAL LPBYTE lpData,
                          IN OUT OPTIONAL LPDWORD lpcbData)
{
    if constexpr (1) {
        ...
        bool(__fastcall * func)(
            void *, void *edx, const char *, OUT OPTIONAL LPDWORD, OUT OPTIONAL LPBYTE, IN OUT OPTIONAL LPDWORD) =
            CAST(func, 0x0081CDC0);
