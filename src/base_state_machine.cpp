...
void base_state_machine::init(state_machine_shared *shared_machine)
{
    ...
    if constexpr (1) {
        ...
        std::for_each(state_list.begin(), state_list.end(), [this](state *the_state) {
            printf("%s %s\n", the_state->get_state_id().to_string(), the_state->get_category_id().to_string());
...
{
    return this->m_curr_state != nullptr && this->m_curr_state->is_flag_set(static_cast<state_flags>(0x800));
}
