                              ...
                              void *a10)
{
    ...
    *allocated_mem = false;
    ...
    if (header->is_flagged(0x40000000)) {
        ...
        assert(addr[0] == MASH_V_TABLE_VAL[0] || addr[0] == ((char *)&virtual_table_lookup[header->class_id])[0]);

        assert(addr[1] == MASH_V_TABLE_VAL[1] || addr[1] == ((char *)&virtual_table_lookup[header->class_id])[1]);

        assert(addr[2] == MASH_V_TABLE_VAL[2] || addr[2] == ((char *)&virtual_table_lookup[header->class_id])[2]);

        assert(addr[3] == MASH_V_TABLE_VAL[3] || addr[3] == ((char *)&virtual_table_lookup[header->class_id])[3]);
