                                                   ...
                                                   tlresource_location *tlres_loc)
{
    ...
    if constexpr (1) {
        assert(my_slot->get_resource_directory().get_tlresource_count(TLRESOURCE_TYPE_MORPH_FILE) ==
               my_slot->get_resource_directory().get_resource_count(RESOURCE_KEY_TYPE_MORPH_FILE_STRUCT));

        if (behavior == UNLOAD) {
            if (tlres_loc->get_data() != nullptr && !nglCanReleaseMorphFile((nglMorphFile *)tlres_loc->get_data())) {
                return true;
            }

        } else if (behavior == LOAD) {
            auto &res_dir = my_slot->get_resource_directory();
            auto idx = this->field_C + res_dir.get_type_start_idxs(RESOURCE_KEY_TYPE_MORPH_FILE_STRUCT);
            ...
            auto alloced_mem =
                parse_generic_object_mash(morphFile, struct_mash, nullptr, nullptr, nullptr, 0, 0, nullptr);
